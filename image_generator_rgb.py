...
def build_vocab(images, vocab_size):
    ...
    total_SIFT_features = np.zeros((20 * len(images) * 3, dim))
    ...
    for channel in range(3):
        for i, img in enumerate(images):
            ...
            total_SIFT_features[i * 20 + channel * 20 * len(images):(i + 1) * 20 + channel * 20 * len(images)] = descriptors[idx, :]
...
def bags_of_sifts(images, vocab_filename):
    ...
    for idx, img in enumerate(images):
        descriptors = []
        for channel in range(3):
            frames, descriptors_sub = vlfeat.sift.dsift(img[:, :, channel], step=4, fast=True)
...
def bags_of_sifts_spm(imgs, vocab_filename, depth):
    ...
    for level in range(depth):
        num_cell += 4 ** level
    feats = np.zeros(shape=(len(imgs), vocab_size * num_cell * 3))

    for idx, img_rbg in enumerate(imgs):
        this_feature = []
        for channel in range(3):
            img = img_rbg[:, :, channel]
            this_feature_sub = []
            for level in range(depth):
                ...
                for index in range(cell_per_line ** 2):
                    ...
                    for d in D:
                        feature[np.argmin(d)] += 1
                    for f in feature:
                        this_feature_sub.append(f * weight)
            this_feature.extend(this_feature_sub)
...
def filter_candidate(candidates, candidates_pos, model):
    filtered_candidates = []
...
def show_results(test_labels, categories, predicted_categories, ax, cmap):
